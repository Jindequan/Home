...
var confPath = "./conf"
var confBakPath = "./confbak"
var generateCommand = &cobra.Command{
	Use:   "config",
	Short: "根据config文件生成struct",
	Long:  `根据config文件生成struct`,
	Run: func(cmd *cobra.Command, args []string) {
		//扫描setting文件夹
		os.Rename(confPath, confBakPath)
		sesstingFds, err := ioutil.ReadDir("./settings")
		if err != nil {
			rollback()
			cmd.PrintErrf("读取settings错误 %v", err)
			return
		}

		regex, err := regexp.Compile(`(.*?)_config(.*?).json`)
		if err != nil {
			rollback()
			cmd.PrintErrf("编译正则失败 %v", err)
			return
		}

		for _, settingFd := range sesstingFds {
			settingName := settingFd.Name()
			res := regex.FindSubmatch([]byte(settingName))
			//匹配成功
			if len(res) > 0 {
				//ex aim_test_a_config.json res[1] : ["aim_test_a"] conf/aim/test/a.go packages : ["aim", "test","a"] path:aim/test genFileName : a.go structName AimTestAConfig   var Name AimTestAConf
				//如果第二个子模式串不为空
				...
				packages := strings.Split(string(res[1]), "_")

				//生成的文件名称 a.go
				genFileName := packages[len(packages)-1] + ".go"
				fmt.Println(genFileName)
				//conf/aim/test
				path := confPath + "/" + strings.Join(packages[:len(packages)-1], "/") + "/"

				structName := ""
				//structName拼接
				for _, pack := range packages {
					structName += pkg.StrFirstToUpper(pack)
				}

				//AimTestAConf
				varName := structName + "Conf"
				//AimTestAConfig
				structName += "Config"
				...
				if len(packages) < 2 {
					...
					pkgName = packages[len(packages)-2]
				}

				os.MkdirAll(path, os.ModePerm)
				configContent, err := ioutil.ReadFile("./settings/" + settingFd.Name())
				if err != nil {
					cmd.PrintErrf("读取配置文件失败 %v", err)
					rollback()
					return

				}

				genConfig, err := Generate(bytes.NewReader(configContent), structName, pkgName, varName,settingFd.Name(),  env,[]string{"json"}, false, true)
				if err != nil {
					cmd.PrintErrf("生成配置文件出错 %v", err)
					rollback()
					return
				}

				err = ioutil.WriteFile(path+genFileName, genConfig, 0644)
				if err != nil {
					cmd.PrintErrf("写入配置文件出错 %v", err)
					rollback()
					return
...
// 生成struct算法原仓库 摘了一部分源码出来用 ～ https://github.com/ChimeraCoder/gojson
func Generate(input io.Reader, structName, pkgName string, varName string, fileName string, env string,tags []string, subStruct bool, convertFloats bool) ([]byte, error) {
	...
	nameAndBak := strings.Split(fileName,".")
	name := nameAndBak[0]
	var underline string
	var ENVContent string
	if env != ""{
		res := strings.Split(name,"config")
		name = res[0] + "config"
		underline = "_"
		ENVContent = `"+pkg.GetEnv()+"`
	}else{
		ENVContent = ""
	}
	...
	path := fmt.Sprintf("./settings/%s%s%s%s",name,underline,ENVContent,".json")

	fn := fmt.Sprintf(`func init() {
	path := "%s"
	pkg.DoNothing()
